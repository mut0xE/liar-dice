...
export function EnterRollup({ game, onReady, }) {
    const { connection } = useConnection();
    const wallet = useAnchorWallet();
    const [status, setStatus] = useState("Preparing…");
    const ran = useRef(false);
    useEffect(() => {
        if (!wallet || ran.current)
            return;
        ran.current = true;
        (async () => {
    ...
    return (_jsxs("main", { className: "screen center", children: [_jsx("h2", { className: "title", children: "ENTER THE ROLLUP" }), _jsx("div", { className: "muted", children: status })] }));
}
